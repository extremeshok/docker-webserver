...
function xshok_upgrade() {
    ...
    if [ "$latest_version" != "" ] ; then
        ...
        if [ "$(printf "%02d%02d%02d%02d" ${latest_version//./ })" -gt "$(printf "%02d%02d%02d%02d" ${script_version//./ })" ] ; then
            ...
            echo "Downloading https://raw.githubusercontent.com/extremeshok/docker-webserver/master/xshok-admin.sh"

            curl --fail --compressed --connect-timeout "30" --remote-time --location --retry "3" --max-time "30" --time-cond "/datastore/xshok-admin.sh.tmp" --output "/datastore/xshok-admin.sh.tmp"  "https://raw.githubusercontent.com/extremeshok/docker-webserver/master/xshok-admin.sh"  2> /dev/null
            ret=$?
            if [ "$ret" -ne 0 ] ; then
                echo "ERROR: Could not download https://raw.githubusercontent.com/extremeshok/docker-webserver/master/xshok-admin.sh"
                exit 1
            fi
            # Detect to make sure the entire script is avilable, fail if the script is missing contents
            if [ "$(tail -n 1 "/datastore/xshok-admin.sh.tmp" | head -n 1 | cut -c 1-7)" != "exit \$?" ] ; then
                echo "ERROR: Downloaded xshok-admin.sh is incomplete, please re-run"
                exit 1
            fi
            # Copy over permissions from old version
            OCTAL_MODE="$(stat -c "%a" "/datastore/xshok-admin.sh")"
...
if ! mv -f "/datastore/xshok-admin.sh.tmp" "/datastore/xshok-admin.sh" ; then
  echo  "ERROR: failed moving /datastore/xshok-admin.sh.tmp to /datastore/xshok-admin.sh"
  rm -f \$0
    exit 1
fi
if ! chmod "$OCTAL_MODE" "/datastore/xshok-admin.sh" ; then
     echo "ERROR: unable to set permissions on /datastore/xshok-admin.sh"
     rm -f \$0
     exit 1
fi
    echo "Completed"
